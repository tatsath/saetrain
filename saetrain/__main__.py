...
class RunConfig(TrainConfig):
    ...
    """Maximum number of tokens to process during training. If None, process all tokens in the dataset."""
...
def run():
    ...
    with nullcontext() if rank == 0 else redirect_stdout(None):
        ...
        if args.max_tokens is not None:
            print(f"Max tokens limit: {args.max_tokens:,}")
        else:
            print("No max tokens limit - will process entire dataset")

        # Create TrainConfig from RunConfig, including the new max_tokens parameter
