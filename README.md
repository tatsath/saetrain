...
```bash
python -m saetrain EleutherAI/pythia-160m <optional dataset>
```
...
The CLI supports all of the config options provided by the `TrainConfig` class. You can see them by running `python -m saetrain --help`.
...
from saetrain import SaeConfig, Trainer, TrainConfig
from saetrain.data import chunk_and_tokenize
...
```bash
python -m saetrain EleutherAI/pythia-160m togethercomputer/RedPajama-Data-1T-Sample --finetune EleutherAI/sae-pythia-160m-32x
```
...
```bash
python -m saetrain gpt2 --hookpoints "h.*.attn" "h.*.mlp.act"
```

To restrict to the first three layers:

```bash
python -m saetrain gpt2 --hookpoints "h.[012].attn" "h.[012].mlp.act"
```
...
```bash
torchrun --nproc_per_node gpu -m saetrain meta-llama/Meta-Llama-3-8B --batch_size 1 --layers 16 24 --k 192 --grad_acc_steps 8 --ctx_len 2048
```
...
```bash
torchrun --nproc_per_node gpu -m saetrain meta-llama/Meta-Llama-3-8B --distribute_modules --batch_size 1 --layer_stride 2 --grad_acc_steps 8 --ctx_len 2048 --k 192 --load_in_8bit --micro_acc_steps 2
```
...
`pip install saetrain`

## Development

Run `pip install -e .[dev]` from the saetrain directory.
...
```bash python -m saetrain gpt2 --hookpoints "h.*.attn" "h.*.mlp.act" --k_decay_steps 10_000```

GroupMax activation function:

```bash python -m saetrain gpt2 --hookpoints "h.*.attn" "h.*.mlp.act" --activation groupmax```

End-to-end training:

```bash python -m saetrain gpt2 --hookpoints "h.*.attn" "h.*.mlp.act" --loss_fn ce```

or

```bash python -m saetrain gpt2 --hookpoints "h.*.attn" "h.*.mlp.act" --loss_fn kl```
